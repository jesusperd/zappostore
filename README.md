...
- **Vender**: carrito con producto simplificado (**Talla + Cantidad + Color libre**), **zonas de diseño** por pestañas Frente/Espalda/Mangas (visión de espejo, notas y **fotos** persistidas en Supabase Storage, bucket privado con URLs firmadas), 3 desplegables **Operación** (venta directa/presupuesto) / **Taller** / **Diseño**, estado por producto, cliente con Cédula/RIF y Dirección, **tasa de cambio compartida** entre todos los vendedores (ya no una por navegador) — todo guardado en Supabase, con folio autogenerado por una secuencia real de Postgres (ya no un contador del navegador, que colisionaba con más de un vendedor).
...
**Migración a Supabase 100% completa** — incluidas Storage de fotos y la tasa de cambio compartida entre vendedores (antes cada navegador tenía la suya). Login, vendedores (con manager de apertura), clientes, cajas, pedidos+pagos, fotos de diseño y la tasa de cambio corren contra Supabase real, con RLS verificada tabla por tabla. La lógica de pagos con reversión (la parte más delicada de todo el proyecto) fue re-verificada end-to-end contra la base real, con el mismo escenario crítico que se auditó en memoria (corregir un pago mal cargado sin dejar dinero fantasma). Durante la migración se encontraron y corrigieron varios bugs reales que solo aparecen contra una base compartida (un id client-side que no era UUID válido, un contador de folio que colisionaba con más de un vendedor, y la tasa de cambio no compartida) — los atraparon los tests automatizados, no una revisión visual. Se verificó también que no quedó ningún dato de prueba en la base.

**Hecho:** todo lo de arriba, más `schema.sql` alineado (v0.8) y corrido contra el proyecto real.
**Sigue (próximo, de producto):** Inventario/Stock, Gastos/Estadísticas, filtrar Resumen/Reportes por fecha en vez de histórico global.
