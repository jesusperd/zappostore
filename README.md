...
- **Vender**: bloqueado para el rol vendedor si no tiene su turno de caja abierto (el master queda exento, no opera caja propia). Carrito con producto simplificado (**Talla + Cantidad + Color libre**), **zonas de diseño** por pestañas Frente/Espalda/Mangas (visión de espejo, notas y **fotos** persistidas en Supabase Storage, bucket privado con URLs firmadas), 3 desplegables **Operación** (venta directa/presupuesto) / **Taller** / **Diseño**, estado por producto, cliente con Cédula/RIF y Dirección, **tasa de cambio compartida** entre todos los vendedores con formato venezolano (punto de miles, coma decimal) — todo guardado en Supabase, con folio autogenerado por una secuencia real de Postgres.
...
**Migración a Supabase 100% completa**, más una tanda de 5 fixes/features de producto: formato venezolano de la tasa de cambio, bloqueo de venta sin caja abierta + modal de resumen al cerrar turno, edición de vendedores (Edge Function `update-vendor`), delivery + comisión por pedido (`pedidos.has_delivery`/`delivery_fee`/`comision_porcentaje`/`comision_monto`), y un dashboard de Home por rol. Los 5 se verificaron end-to-end en navegador real contra Supabase (incluida la limpieza de los datos de prueba generados).
